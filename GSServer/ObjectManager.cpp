...
void UpdateViewList(unsigned int id, ObjectMap& map)
{
	...
	for (auto& otherID : nearList) {
		...
		if (isInserted && amIPlayer) {
			...
			networkManager.SendNetworkMessage(((Client&)me).GetSocket(), *new MsgPutObject{ other.GetID(), x, y, other.GetColor() });
		}
		...
		if (isPlayer)
		{
			if (!amIInserted) {
				...
				auto[x, y] = me.GetPos();
				networkManager.SendNetworkMessage(((Client&)other).GetSocket(), *new MsgPutObject{ me.GetID(), x, y, me.GetColor() });
			}
		}
		else {
			auto& npc = (NPC&)other;
